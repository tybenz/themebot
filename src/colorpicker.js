(function( $, WebPro, window, document, undefined ) {
  ...
  ThemeBot.Colorpicker = function( $element, options ) {
    ...
    this.offsetX = this.options.offsetX - this.$element.closest('#tmb-side-bar').offset().left;
    this.offsetY = this.options.offsetY - this.$element.closest('#tmb-side-bar').offset().top;
  ...
  ThemeBot.Colorpicker.prototype = {
    init: function() {
      ...
      this.graph.$track.on( 'mousedown', function( evt ) {
        self._jumpThumb( 'graph', evt );
      });
      ...
      this.hue.$track.on( 'mousedown', function( evt ) {
        self._jumpThumb( 'hue', evt );
      });

      this.$element.on( 'mousedown click mouseup', function( evt ) {
        evt.preventDefault();
      });
    ...
    _colorInput: function() {
        var color = this._convert({ h: this.value.h, s: this.value.s, b: this.value.b }, "hsb2hex" );
        if ( this.$input ) {
          this.$input.css({
            'background-color': color,
            'color': this._greyValue( color ) > 100 ? '#000' : '#fff'
          });
        }
    },

    _jumpThumb: function( sliderType, evt ) {
      slider = sliderType == 'hue' ? this.hue : this.graph;
      if ( sliderType == 'graph' ) {
        slider.setPositionByPixel( evt.pageX - slider.$track.offset().left - 10, evt.pageY - slider.$track.offset().top - 10 );
      } else {
        slider.setPositionByPixel( evt.pageX - slider.$track.offset().left, evt.pageY - slider.$track.offset().top );
      }
      slider.tracker.dragStarted = true;
      slider.tracker._startDrag( evt );
    }
  };
